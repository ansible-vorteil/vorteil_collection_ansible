...
class VorteilClient(object):
    ...
    # Short description: Download the package of an app from a remote Vorteil.io Repo
    # Module Parameters:
    #       repo_app:               application to query within the bucket
    #       repo_bucket:            bucket to query for applications
    #       package_save_location:  save location for app package
    def download_app_package(self):
        param = 'graphql?query=query{bucket(name:"' + self.module.params['repo_bucket'] + '"){ ' + \
            'app(name:"' + self.module.params['repo_app'] + '") { latest { file { downloadURL } } } } }'
        url = '{0}/{1}'.format(self.repo_url, param)
        headers = {
            'accept': "application/json",
            'content-type': "application/json"
        ...
        module_response = response.json()['data']
        module_response['package_save_location'] = self.module.params['package_save_location']

        # Download package
        downloadURL = response.json()['data']['bucket']['app']['latest']['file']['downloadURL']
        downloadURL = '{0}{1}'.format(self.repo_url, downloadURL)
        packageResponse = requests.request("GET", downloadURL, headers=headers, verify=False, cookies=self.repo_cookie)

        if packageResponse.status_code != 200:
            return Exception('GET /api/signed/ {0}'.format(packageResponse.status_code)), True

        with open(module_response['package_save_location'], 'wb') as filehandle:
            try:
                filehandle.write(packageResponse.content)
                return module_response, False
            except Exception:
                return ('Could not create package file:{0}'.format(module_response['package_save_location'])), True
    ...
    # Short description: create the injection URI for provisioning the disk build process from a germ
    # Module Parameters:
    #       vorteil_germ:       germ of vorteil app
    #       repo_image_name:    name of image that is provisioned
    #       repo_provisioner:   configured provisioner to provision to
    def provision_injection_uri_germ(self):
        ...
        if self.module.params['skip_injection']:
            param = 'graphql?query=mutation{provision(germ: "' + self.module.params['vorteil_germ'] + \
                    '", injections: [], name: "' +\
                    self.module.params['repo_image_name'] + '" , provisioner: "' +\
                    self.module.params['repo_provisioner'] + '"){uri,job{id}}}'
        else:
            param = 'graphql?query=mutation{provision(germ: "' + self.module.params['vorteil_germ'] + \
                    '", injections: ["' + repo_uuid + '"], name: "' +\
                    self.module.params['repo_image_name'] + '" , provisioner: "' +\
                    self.module.params['repo_provisioner'] + '"){uri,job{id}}}'
        ...
        if self.module.params['wait_until_finished']:
            jobFinished = None
            jobResponse = None
